...
import ek.sdet.framework.pages.POMFactory;
...
Assert.assertTrue(isElementEnabled(getDriver().findElement(By.xpath("//span[text() = '" + optionsToDisplay.get(0).get(0) + "']" ))));
...
switch (department) {
case "Electronics":
Assert.assertTrue(isElementEnabled(factory.homePage().electronics));
logger.info("user is on Electronics section");
break;

case "Computers":
Assert.assertTrue(isElementEnabled(factory.homePage().computer));
logger.info("user is on Computers section");
break;

case "Smart Home":
Assert.assertTrue(isElementEnabled(factory.homePage().smarthome));
logger.info("user is on Smart Home section");
break;

case "Sports":
Assert.assertTrue(isElementEnabled(factory.homePage().Sports));
logger.info("user is on Sports section");
break;

case "Automotive":
Assert.assertTrue(isElementEnabled(factory.homePage().Automative));
...
Assert.assertTrue(isElementEnabled(factory.homePage().cartBtn));
...
Assert.assertTrue(isElementEnabled(factory.homePage().Thanks));
